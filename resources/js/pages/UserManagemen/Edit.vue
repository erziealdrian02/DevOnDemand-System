...
interface AdditionalFormItem {
    id: number;
    assignment_id?: string; // For existing assignments
    start_date: string;
    end_date: string;
    notes: string;
    employee_id: string;
    attachment: File | null;
    existing_attachment?: string | null; // For displaying existing attachments
...
const form = ref({
    ...
    // For tracking deleted assignments
    deleted_assignments: [] as string[],
});

// Format date for input fields
const formatDateForInput = (dateString: string | null) => {
    if (!dateString) return '';
    // If already has 'T' (local datetime format), return as is
    if (dateString.includes('T')) return dateString;

    // Otherwise, try to convert to local datetime format
    try {
...
const employeeCount = computed(() => {
    ...
    const filledEmployeeIds = form.value.additionalItems.filter((item) => !item._delete && item.employee_id && item.employee_id.trim() !== '').length;
...
const removeFormItem = (index: number) => {
    const item = form.value.additionalItems[index];

    if (item.assignment_id) {
        // If it's an existing assignment, mark for deletion
        form.value.deleted_assignments.push(item.assignment_id);
        form.value.additionalItems[index]._delete = true;
    } else {
        // If it's a new assignment, remove it from the array
        form.value.additionalItems.splice(index, 1);
    }

    // Remove dropdown state
    if (employeeDropdowns.value[item.id]) {
        delete employeeDropdowns.value[item.id];
    }
};
...
const initializeForm = () => {
    ...
    // Initialize assignments if project is approved
    if (project.is_approved && page.props.assignments) {
        form.value.additionalItems = page.props.assignments.map((assignment) => {
            const itemId = nextFormItemId++;

            // Initialize dropdown for this item
            initEmployeeDropdown(itemId);
            ...
            return {
                ...
                attachment: null,
                existing_attachment: assignment.attachment,
            };
        });
    }

    // Add one form item if approved but no assignments
    if (project.is_approved && form.value.additionalItems.length === 0) {
        addNewFormItem();
    }
};

// Initialize with one form item when approved changes
watch(
...
const submit = () => {
    ...
    // Append deleted assignments
    if (form.value.deleted_assignments.length > 0) {
        formData.append('deleted_assignments', JSON.stringify(form.value.deleted_assignments));
    }

    // Append additional items if approved
    if (form.value.is_approved) {
        // Only include non-deleted items
        const activeItems = form.value.additionalItems.filter((item) => !item._delete);

        // Create a clean version of additionalItems without the file objects
        const additionalItemsForJSON = activeItems.map((item) => ({
            id: item.id,
            assignment_id: item.assignment_id, // Include if exists
    ...
    router.put(`/projects/${form.value.id}`, formData, {
        onSuccess: () => {
        ...
        <div class="flex flex-1 flex-col gap-4 rounded-xl p-12">
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 gap-6 rounded-2xl border p-6 md:grid-cols-[1fr_1px_1fr]">
                ...
                <div v-if="form.is_approved" class="col-span-1 mt-6 space-y-6 md:col-span-3">
                    ...
                    <div
                        v-for="(item, index) in form.additionalItems"
                        :key="item.id"
                        v-show="!item._delete"
                        class="relative space-y-4 rounded-lg border p-4"
                    >
                        <!-- Remove button -->
                        <button type="button" @click="removeFormItem(index)" class="absolute right-2 top-2 text-red-500 hover:text-red-700">
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div class="space-y-2">
                                ...
                                <div v-if="item.existing_attachment" class="mt-1 text-sm text-blue-500">
                                    <a :href="item.existing_attachment" target="_blank" class="flex items-center gap-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path
